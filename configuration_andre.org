...
On Ubuntu I have mapped Meta to Windows key, so that IntelliJ settings shortcut etcetera keep functioning well. To make sure that emacs can use Alt as meta, do this.
#+begin_src emacs-lisp
;; Map Alt key to Meta
...
** helm-tramp for connecting via ssh to servers or docker containers.
#+BEGIN_SRC emacs-lisp 
...
** Macros
#+BEGIN_SRC lisp
(fset 'enclose-postman-vars
   (lambda (&optional arg) "Keyboard macro." (interactive "p") (kmacro-exec-ring-item (quote ([201326629 32 123 123 46 42 5 return 34 124 124 left 92 38 return 33] 0 "%d")) arg)))

(fset 'declose-postman-vars
   (lambda (&optional arg) "Keyboard macro." (interactive "p") (kmacro-exec-ring-item (quote ([201326629 34 11 124 92 124 124 34 left left left left 32 5 return return 33] 0 "%d")) arg)))
