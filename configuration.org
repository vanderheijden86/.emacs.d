...
Remap some handy keybindings defined in emp-keybindins because
org-mode overwrites them.

#+BEGIN_SRC emacs-lisp
(with-eval-after-loadd "org" (define-key org-mode-map (kbd "C-k") nil))
;;(with-eval-after-load "org "(define-key org-mode-map "\C-k" nil))
...
** Load environment from .bashrc to emacs.
...
enabled. Disabled this now .emacs.d folder is not yet symlinked.
